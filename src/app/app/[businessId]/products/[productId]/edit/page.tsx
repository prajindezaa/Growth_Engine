...
export default function EditProductPage() {
  ...
  async function loadData() {
    ...
    const [{ data: prd }, { data: sups }] = await Promise.all([
      supabase.from("products").select("*").eq("id", productId).eq("business_id", businessId).single(),
      supabase.from("suppliers").select("id, name").eq("business_id", businessId).order("name"),
    ]);
    if (sups) setSuppliers(sups as Supplier[]);
    if (prd) {
      const p = prd as Product;
      setOriginal(p);
      setForm({
        name: p.name, sku: p.sku || "", barcode: p.barcode || "",
        category: p.category || "", unit: p.unit,
        purchase_price: p.purchase_price, selling_price: p.selling_price, tax_rate: p.tax_rate,
        current_stock: p.current_stock, min_stock: p.min_stock, max_stock: p.max_stock,
        supplier_id: p.supplier_id || "", hsn_sac: p.hsn_sac || "", is_active: p.is_active,
      });
    }
    setLoading(false);
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const supabase = createClient();
    let image_url = original?.image_url || null;
    ...
    const { error: err } = await supabase.from("products").update({
      ...
      purchase_price: form.purchase_price, selling_price: form.selling_price, tax_rate: form.tax_rate,
      current_stock: form.current_stock, min_stock: form.min_stock, max_stock: form.max_stock,
      supplier_id: form.supplier_id || null, hsn_sac: form.hsn_sac || null,
      image_url, is_active: form.is_active,
    }).eq("id", productId);
    ...
    await supabase.rpc("insert_audit_log", {
      p_business_id: businessId, p_action: "update", p_entity_type: "product",
      p_entity_id: productId,
      p_before: original ? { name: original.name, sku: original.sku, selling_price: original.selling_price } : null,
      p_after: { name: form.name, sku: form.sku, selling_price: form.selling_price },
    });

    router.push(`/app/${businessId}/products/${productId}`);
  }
  ...
  return (
    <div style={{ padding: "40px", maxWidth: "640px" }}>
      <div className="ge-animate-in">
        <h1 style={{ fontSize: "1.5rem", fontWeight: 700, color: "var(--ge-text-primary)", letterSpacing: "-0.02em", marginBottom: "4px" }}>Edit Product</h1>
        <p style={{ fontSize: "0.875rem", color: "var(--ge-text-secondary)", marginBottom: "32px" }}>Update {original?.name || "product"} details.</p>
        ...
        <form onSubmit={handleSubmit}>
          <Section title="Basic Info">
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "12px" }}>
              <div>
                ...
                <input type="text" value={form.sku} onChange={(e) => update({ sku: e.target.value })} className="ge-input" />
              </div>
              <div>
                <label className="ge-label">Barcode</label>
                <input type="text" value={form.barcode} onChange={(e) => update({ barcode: e.target.value })} className="ge-input" />
              </div>
              <div>
                <label className="ge-label">Unit</label>
                <select value={form.unit} onChange={(e) => update({ unit: e.target.value })} className="ge-input">
                  <option value="pcs">Pieces</option><option value="kg">Kilograms</option><option value="g">Grams</option>
                  <option value="l">Litres</option><option value="ml">Millilitres</option><option value="m">Metres</option>
                  <option value="box">Box</option><option value="pack">Pack</option><option value="dozen">Dozen</option>
                </select>
              </div>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
              <div>
                ...
                <input type="text" value={form.category} onChange={(e) => update({ category: e.target.value })} className="ge-input" />
              </div>
              <div>
                <label className="ge-label">HSN/SAC</label>
                <input type="text" value={form.hsn_sac} onChange={(e) => update({ hsn_sac: e.target.value })} className="ge-input" />
              </div>
            </div>
          </Section>
          ...
          <Section title="Supplier & Image">
            ...
            <div>
              <label className="ge-label">Product image {original?.image_url ? "(replace)" : ""}</label>
              {original?.image_url && <img src={original.image_url} alt="" style={{ width: "64px", height: "64px", borderRadius: "var(--ge-radius)", objectFit: "cover", marginBottom: "8px" }} />}
              <input type="file" accept="image/*" onChange={(e) => setImageFile(e.target.files?.[0] || null)} className="ge-input" style={{ padding: "10px" }} />
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
              <input type="checkbox" checked={form.is_active} onChange={(e) => update({ is_active: e.target.checked })} id="is_active" style={{ accentColor: "var(--ge-accent)" }} />
